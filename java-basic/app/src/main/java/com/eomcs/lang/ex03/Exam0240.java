...
// - 인간의 연산 방식과 다르게 동작한다.
// - 빼기를 처리하는 컴퓨팅 회로(CPU)를 별도로 설계해야 하므로 하드웨어(CPU)가 복잡해진다.
// - 장점
// - 사람이 봤을 때 이해하기 쉽다!
//
// 2) 1의 보수(Ones' Complement)
// - 1의 보수 : 1이 되기위해 보충해주는 수(각 비트마다 1이 되기위해 필요한 값)
// -> 모든 비트를 반대 값으로 바꾼다.
// 예) +24 => 0001 1000
// 예) -24 => 1110 0111
// - 수의 범위(8비트 기준): -127 ~ +127
...
// (1의 보수 문제점인 1을 더해야하는 단점으로 인해 1의 보수를 구하고 미리 1을 더하는 방식)
// - 음수 0을 없앰으로써 -128까지 표현할 수 있음.
// (음수 0의 값에 1을 더하면 양수 0의 값이 됨)
// - 2의 보수를 만드는 방법 1:

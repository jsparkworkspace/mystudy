...
public class Exam0112 {
  ...
  public static void main(String[] args) throws Exception {
    // 메서드의 의미 - JVM에서 외부로 데이터를 내보내거나 외부에서 데이터를 읽을 때 사용하는 문자 규칙
    // Charset.defaultCharset() : Charset 클래스의 defaultCharset() 메서드를 호출
    // -> JVM은 내부적으로 무조건 UTF-16BE를 사용하고 있다.
    // -> defaultCharset()은 OS가 사용하고있는 기본 캐릭터셋을 반환해준다.
    // - * JVM이 문자를 출력할 때는 OS의 defaultCharset으로 인코딩하여 출력한다.

    System.out.println(Charset.defaultCharset());

    // 한글 문자 코드의 바이트 배열을 가지고 String 인스턴스 초기화시키기.
    byte[] bytes = {(byte) 0xb0, (byte) 0xa1, // 가 (EUC-KR 코드)
